...
(function renderData() {
  ...
  // About: column layout
  var aboutCols = document.getElementById('about-paragraphs');
  if (aboutCols) {
    aboutCols.innerHTML = d.profile.about.map(function (t) { return '<p>' + t + '</p>'; }).join('');
  }
  ...
  function renderArticles(gridId, items) {
    var grid = document.getElementById(gridId);
    if (!grid) return;
    grid.innerHTML = items.map(function (item, i) {
      return '<article class="work-article reveal">' +
        '<div class="work-num">No. ' + String(i + 1).padStart(2, '0') + '</div>' +
        '<h3>' + item.title + '</h3>' +
        '<p>' + item.comment + '</p>' +
  ...
  renderArticles('anime-grid', d.anime);
  renderArticles('movies-grid', d.movies);
